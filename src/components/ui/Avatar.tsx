...
export function Avatar({ user, size = "sm", className = "" }: { user?: UserProfile; size?: keyof typeof SIZES; className?: string }) {
  // Remember which URL failed so a new avatar URL gets a fresh attempt.
  ...
  return (
    ...
    >
      {showImage ? (
        ...
        <img src={user!.avatarUrl} alt="" referrerPolicy="no-referrer" onError={() => setFailedUrl(user!.avatarUrl ?? null)} className="w-full h-full object-cover" />
      ) : (
        initial
      )}
    </div>
